<template>
  <div class="home_container">
    ...
    <div class="lg:text-24px text-16px text-FBFF00 font-bold mt-2.5 main_container mx-auto px-4 lg:px-0 md:mt-0">FBT质押奖励池，每24小时最高10%的质押奖励和13%的直推奖励</div>
    <div class="flex flex-col lg:flex-row main_container mx-auto mt-8 space-y-4 lg:space-y-0 lg:space-x-4 px-4 lg:px-0">
      <!-- card1 -->
      <div class="card1 p-4 lg:w-1/2">
        <div class="flex flex-row justify-between">
          <div class="fStyle18_3D3D3D_bold">总矿池</div>
          <div class="fStyle18_FFED4C_bold">{{totalSupply}} FBT</div>
        </div>
        <div class="flex flex-row justify-between mt-2">
          <div class="fStyle18_3D3D3D_bold">钱包</div>
          <div class="fStyle18_FFED4C_bold">{{fbtContract && fbtContract.balanceOf || '0' | subNumber}} FBT</div>
        </div>
        <div class="flex flex-row justify-between mt-2">
          <div class="fStyle18_3D3D3D_bold">你的算力</div>
        ...
        <button class="round_button button mt-4" @click="burn">燃烧FBT</button>
        <div class="flex flex-row justify-between mt-4">
          <div class="fStyle14_3D3D3D_bold">日利率</div>
          <div class="fStyle14_FFED4C_bold">10%</div>
        </div>
        <div class="flex flex-row justify-between mt-2">
          <div class="fStyle14_3D3D3D_bold">年利率</div>
          <div class="fStyle14_FFED4C_bold">3650%</div>
        </div>
        <div class="flex flex-row justify-between mt-2">
          <div class="fStyle14_3D3D3D_bold">服务费</div>
          <div class="fStyle14_FFED4C_bold">3%</div>
        </div>
      </div>
      <!-- card2 -->
      <div class="card2 p-4 lg:w-1/2">
        <div class="fStyle24_FBFF00_bold text-left">推广链接</div>
        <div class="fStyle14_FFFFFF_bold mt-2.5 text-left">从使用您的推荐链接的任何人那里获得用于燃烧FBT的13%</div>
        <div class="flex flex-row justify-between mt-6">
          <div class="input_wrap s_input_wrap flex flex-row justify-between items-center px-4 ellipsis">
            <div>{{shareUrl()}}</div>
          </div>
          <button class="button midlle_button ml-10" @click="h5Copy(shareUrl())" style="white-space:nowrap">复制</button>
        </div>
        <div class="h_dot_line"></div>
        <div class="flex flex-row justify-between mt-2">
          <div class="fStyle18_FFFFFF_bold">你的产量:</div>
          <div class="fStyle18_FFFFFF_bold scale_text">{{reward}} FBT</div>
          <!-- <div class="fStyle18_FFFFFF_bold">{{reward }} BNB</div> -->
        </div>
        <div class="flex flex-row justify-between mt-8">
          <button class="midlle_button button" @click="rebuy">继续燃烧</button>
          <button class="midlle_button button" @click="getReceiveIncome">提现</button>
        </div>
      </div>
      <!-- card3 -->
...
import { abi, NORMAL_ABI } from "./abi";
...
import { amount2Wei} from '@/utils/ethersUtils/helper/number'
...
export default {
  data() {
    return {
      ...
      fbt: process.env.VUE_APP_FBT, 
      contract: null, // 当前的合约对象
      erc20Contract: null, // ERC20合约对象
      myAddress: "", // 我的地址
      balance: "0.00", // 我的余额
  ...
  async created() {
    ...
    this.fbtContract = await new ContractUtils(this.fbt, NORMAL_ABI, this.provider.signer, this.provider.address, this.provider.gasPrice, this.provider.provider)
  ...
  methods: {
    ...
    async getTotalSupply() {
      var num = await this.contract.methodWithDecimal('getBalance', this.fbtContract.decimal);
      this.totalSupply= num[1]
    },
    async getCoinAmount() {
   ...
   //我的奖励
    async getreward() {
      ...
      console.log('getreward====',error, eggs)
      if(eggs != 0) {
        let [, res] = await this.contract.methodWithDecimal('calculateEggSell', this.fbtContract.decimal, [eggs])
        if(error == null) {
          this.reward = res
        }
      } else {
    ...
    async burn() {
      ...
      let amount = amount2Wei(this.amount, this.fbtContract.decimal);
      this.submtting = true
      let [error,] = await this.fbtContract.approveMethod(this.contractAddress, this.fbtContract.decimal, this.amount, async () => {
        return await this.contract.estimateMethod('buyEggs', ()=>{
          this.amount = "";
          this.submtting = false
          this.getTotalSupply()
          this.fbtContract.getBalance()
          Toast("成功");
          return [null]
        }, [this.queryAddress, amount])
      })  

      console.log('burn=====',error, this.queryAddress, amount)
